...
@section('content')
    <div class="flex backdrop-blur-md border border-white/20 shadow-lg rounded-2xl mx-[80px] py-[32px] px-[80px] flex-col gap-[32px]">
        ...
        @if($negotiation->is_agen_approve === null && $negotiation->is_seller_approve === null && $negotiation->agent_id === Auth::id())
        ...
        <div class="w-full flex gap-4">
                <div class="w-full">
                    <form action="{{ route('agent.approveNegotiation', ['id' => $negotiation->id]) }}" method="POST">
                        @csrf
                        @method('PATCH')
                ...
                <div class="w-full">
                    <a href="{{ route('agent.negotiationRejectionReason', ['id' => $negotiation->id]) }}">
